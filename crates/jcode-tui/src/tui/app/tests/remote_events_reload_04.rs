...
#[test]
fn remote_account_switch_keeps_verified_claude_context_limit() {
    let mut app = create_test_app();
    app.is_remote = true;
    app.remote_provider_name = Some("Claude".to_string());
    app.remote_provider_model = Some("claude-opus-4-8".to_string());
    app.update_context_limit_for_model("claude-opus-4-8");
    assert_eq!(app.context_limit, 1_000_000);

    // The account-switch reset used to write the inert provider's 200K
    // default into the footer, and the follow-up catalog event (same model)
    // never re-resolved it (replace_remote_model_catalog_snapshot only
    // re-resolves when the model differs). It must instead re-resolve the
    // live session model so a 1M-window model keeps its real window.
    app.refresh_context_limit_for_current_model();
    assert_eq!(app.context_limit, 1_000_000);

    // Fable 5 takes the same path.
    app.remote_provider_model = Some("claude-fable-5".to_string());
    app.update_context_limit_for_model("claude-fable-5");
    assert_eq!(app.context_limit, 1_000_000);
    app.refresh_context_limit_for_current_model();
    assert_eq!(app.context_limit, 1_000_000);

    // A 200K-capped generation keeps its real window across the refresh.
    app.remote_provider_model = Some("claude-sonnet-4-5".to_string());
    app.update_context_limit_for_model("claude-sonnet-4-5");
    app.refresh_context_limit_for_current_model();
    assert_eq!(app.context_limit, 200_000);
}

#[test]
fn remote_account_switch_without_catalog_keeps_existing_limit() {
    let mut app = create_test_app();
    app.is_remote = true;
    let before = app.context_limit;
    app.refresh_context_limit_for_current_model();
