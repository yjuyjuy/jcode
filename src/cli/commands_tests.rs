...
// ("account exhausted ..., rotated to claude-2, ...") is built from these rows,
// so the wording here is load-bearing.
...
fn render_switch_outcome_line_carries_unknown_label_error() {
    // The unknown-label refusal path: the provider runtime rejects a label that
    // is not in auth.json, and its reason must reach the operator verbatim.
    let line = render_switch_outcome_line(&switch_outcome(
        "sess-d",
        false,
...
fn switch_results_any_failed_flags_a_single_failure() {
    // One unknown-label refusal in an --all sweep must fail the whole command so
    // the orchestrator's exit-status gate catches it.
    let results = vec![
...
fn switch_results_any_failed_treats_empty_as_no_failure() {
    // No live session matched: an empty result set is not a failure, so `--all`
    // on an idle fleet exits zero rather than erroring.
